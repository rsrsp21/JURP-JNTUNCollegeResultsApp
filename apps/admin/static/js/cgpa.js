...
async function displayResults() {
    ...
    // Set up backend-provided academic summary
    var academicSummary = studentData.academicSummary || {};
    var regulation = studentData['Regulation'];
    var progressPercentage = Number(academicSummary.progressPercentage || 0);
    if (!Number.isFinite(progressPercentage)) progressPercentage = 0;
    progressPercentage = Math.max(0, Math.min(100, progressPercentage));
    progressFill.style.width = `${progressPercentage}%`;
    progressFill.className = `progress-bar-fill ${academicSummary.progressClass || 'pass-class'}`;

    // Display division message
    var message = academicSummary.division || 'N/A';

    messageContainer.innerHTML = '';
    if (message !== '') {
        var messageElement = document.createElement('div');
        messageElement.textContent = message;
        messageElement.className = `division-badge ${academicSummary.divisionClass || 'not-applicable'}`;
    ...
    percentageValue.textContent = academicSummary.percentage || 'N/A';
    ...
    var suppCount = Number(academicSummary.supplementaryCount || 0);
    if (!Number.isFinite(suppCount)) suppCount = 0;
