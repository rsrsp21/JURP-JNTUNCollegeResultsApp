...
def _extract_new_model_from_text(pdf):
    ...
    page_stats = []
    current_id = None
    # Name fragments accumulate independently of where the SubCode line lands
    # (see below) rather than a single prefix-anchored pending_line string.
    ...
    # subcode-stripped, fully-joined name (see pending_parts below).
    ...
    for page_num, page in enumerate(pdf.pages, 1):
        ...
        for line in lines:
            ...
            # A tall wrapped-name cell can put the SNo/SubCode line anywhere
            # among the name's wrapped lines (top-aligned, vertically
            # centered, etc.), not necessarily first. So rather than assuming
            # the SubCode always leads pending text, extract it whenever it
            # appears and keep collecting name fragments in the order seen,
            # independent of where the SubCode line landed.
            was_just_added = just_added_record
            just_added_record = False
            ...
            if subcode_match:
                ...
                if active:
                    ...
                    # Likely the start of the NEXT row's wrapped name, whose
                    # SNo/SubCode line hasn't appeared yet (see above) -
                    # rather than a trailing continuation of the row we just
                    # finished, which is what this used to assume.
                    pending_parts = [line]
                else:
                    # Nothing in progress - likely header/footer noise that
                    # slipped past the keyword filters above.
                    continue
...
def sync_semester_to_flask(batch, semester, base_path):
    """
    Finds the latest processed semester CSV in base_path and syncs it
    to the portal D1 database.
    """
    # Mapping semesters
    mapping = {
        "1-1": "1", "1-2": "2", "2-1": "3", "2-2": "4",
    ...
    try:
        ...
        rows = replace_semester_from_dataframe(
            batch,
            int(sem_num),
            df,
            is_honors_minor=(semester == "honors-minors")
        )
        return True, f"Successfully synchronized semester {semester} to D1 semester_results ({rows} rows)"
    except Exception as e:
        return False, f"Failed to sync semester file to D1: {str(e)}"
...
def sync_cgpa_to_flask(batch, base_path):
    ...
    and writes it to the portal D1 database.
    """
    ...
    try:
        ...
        df = df[standard_cols]
        rows = replace_cgpa_from_dataframe(batch, df)
        return True, f"Successfully synchronized CGPA master report to D1 student_cgpa ({rows} rows)"
    except Exception as e:
        return False, f"Failed to sync CGPA to D1: {str(e)}"

def sync_toppers_to_flask(batch, base_path):
    """
    Copies top_10_students.csv into the portal D1 toppers table.
    """
    ...
    try:
        df = pd.read_csv(source_file)
        rows = replace_toppers_from_dataframe(batch, df)
        return True, f"Successfully synchronized toppers list to D1 toppers ({rows} rows)"
    except Exception as e:
        return False, f"Failed to sync toppers to D1: {str(e)}"
